...
public:
    RawDatabase(const QString& path, const QString& password, const uint8_t salt[]);
    ~RawDatabase();
    bool isOpen();
...
protected:
    static QString deriveKey(const QString &password, const uint8_t salt[TOX_PASS_SALT_LENGTH]);
...
private:
    ...
    uint8_t currentSalt[TOX_PASS_SALT_LENGTH];
};
