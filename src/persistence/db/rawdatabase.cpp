...
bool RawDatabase::openEncryptedDatabaseAtLatestSupportedVersion(const QString& hexKey)
{
    ...
    // could be upgraded to SQLCipher 4.0 defaults, since SQLCipher 3.x isn't capable of setitng all the same params.
    // If SQLCipher 4.x happened to be used, they would have been fully upgraded to 4.0 default params.
...
{
    QString libVersion{sqlite3_libversion()};
    auto majorVersion = libVersion.split('.')[0].toInt();
