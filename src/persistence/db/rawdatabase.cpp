...
{
    // old qTox database are saved with SQLCipher 3.x defaults. Some for a period during qTox 1.16.3, Arch linux updated sqlcipher to 4.x
    // so some new profiles were saved with 4.x defaults, since at the time qTox didn't specify cipher params. We need to support
    // opening both databases saved with 3.x defaults and 4.x defaults, so upgrade to 3.x default to 4.x defaults while we're at it
    if (!setKey(hexKey)) {
        return false;
    }
    ...
    if (setCipherParameters(4)) {
        if (testUsable()) {
            ...
            return upgradeFrom3To4Parameters(hexKey);
        }
    } else {
...
bool RawDatabase::upgradeFrom3To4Parameters(const QString& hexKey)
{
