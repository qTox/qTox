    ...
    forever
    {
        ...
        for (Query& query : trans.queries)
        {
            for (sqlite3_stmt* stmt : query.statements)
            {
                ...
                QString query = anonymizeQuery(query.query);
                switch (result) {
                case SQLITE_ERROR:
                    qWarning() << "Error executing query" << query;
                    goto cleanupStatements;
                case SQLITE_MISUSE:
                    qWarning() << "Misuse executing query" << query;
                    goto cleanupStatements;
                case SQLITE_CONSTRAINT:
                    qWarning() << "Constraint error executing query" << query;
                    goto cleanupStatements;
                default:
...
QString RawDatabase::anonymizeQuery(const QByteArray& query)
{
    ...
    queryString.replace(QRegExp("chat.public_key='[A-Z0-9]{64}'"),
                        "char.public_key='<HERE IS PUBLIC KEY>'");
    queryString.replace(QRegExp("timestamp BETWEEN [0-9]{1,} AND [0-9]{1,}"),
