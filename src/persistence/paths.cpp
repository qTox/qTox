 ...
 * @param forcePortable if true, force portable mode
 * @param forceNonPortable if true, force non-portable mode
 * @return Pointer to Paths object on success, nullptr else
 * @note forcePortable and forceNonPortable can't be true at the same time
 *       if both of them are false, we autodetect if portable mode should be used
 */
Paths* Paths::makePaths(bool forcePortable, bool forceNonPortable)
{
    if (forcePortable && forceNonPortable) {
        qCritical() << "Can't be portable and non portable at the same time";
        return nullptr;
    }
