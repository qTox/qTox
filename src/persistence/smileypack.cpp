...
QString SmileyPack::smileyfied(const QString& msg)
{
    ...
    for ( auto r = emoticonToPath.begin(); r != emoticonToPath.end(); ++r) {
        ...
        if (r.key().toUcs4().length() == 1) {
            ...
            // pattern like ":)" or ":smile:", don't match inside a word or else will hit punctuation and html tags
            exp.setPattern(QStringLiteral(R"((?<=^|[\s\n]))") + QRegularExpression::escape(r.key()) + QStringLiteral(R"((?=$|[\s\n]))"));
        }
        int replaceDiff = 0;
