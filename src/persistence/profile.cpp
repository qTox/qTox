                              ...
                              const QCommandLineParser* parser)
{
    ...
    QString path = settings.getPaths().getLegacySettingsDirPath() + name + ".tox";
                                ...
                                const QCommandLineParser* parser)
{
    ...
    QString path = Settings::getInstance().getPaths().getLegacySettingsDirPath() + name + ".tox";
...
{
    QDir dir(Settings::getInstance().getPaths().getLegacySettingsDirPath());
    QStringList out;
...
QString Profile::avatarPath(const ToxPk& owner, bool forceUnencrypted)
{
    ...
    if (!encrypted || forceUnencrypted) {
        return Settings::getInstance().getPaths().getLegacySettingsDirPath() + "avatars/" + ownerStr + ".png";
    }
    ...
    return Settings::getInstance().getPaths().getLegacySettingsDirPath() + "avatars/" + hash.toHex().toUpper() + ".png";
}
...
void Profile::saveAvatar(const ToxPk& owner, const QByteArray& avatar)
{
    ...
    QString path = avatarPath(owner);
    QDir(Settings::getInstance().getPaths().getLegacySettingsDirPath()).mkdir("avatars");
    if (pic.isEmpty()) {
        QFile::remove(path);
    } else {
        QSaveFile file(path);
...
{
    QString path = Settings::getInstance().getPaths().getLegacySettingsDirPath() + name;
...
{
    QString path = Settings::getInstance().getPaths().getLegacySettingsDirPath() + name,
            newPath = Settings::getInstance().getPaths().getLegacySettingsDirPath() + newName;
...
{
    return Settings::getInstance().getPaths().getLegacySettingsDirPath() + profileName + ".db";
}
