...
namespace {
const QString globalSettingsFile = QStringLiteral("qtox.ini");
Settings* settings{nullptr};
QMutex bigLock{QMutex::Recursive};
QThread* settingsThread{nullptr};
const char* autoAcceptUpgradeMessage =
...
const char* friendAutoAcceptUpgradeMessage =
    "In an attempt to protect your file transfer history qTox has changed the default "
    "per-friend download directory to an application specific directory. Would you like to switch "
    "your current downloads directory to the new default?\n";
} // namespace
...
void Settings::loadGlobal()
{
    ...
    s.beginGroup("General");
    {
        ...
        auto onSettingsUpgrade = [&](UpgradableSettingResponse<QString, QString> const& response) {
            if (response.isUpgraded()) {
                globalAutoAcceptDir = response.asNewType();
            } else {
                globalAutoAcceptDir = response.asOldType();
            }
        };

        auto autoAcceptUpgrader = UpgradableSetting<QString, QString>::Builder()
                                      ...
                                      .upgradeMessage(QString(tr(autoAcceptUpgradeMessage)))
                                      .defaultConstructedIsNull()
                                      .build();

        autoAcceptUpgrader.addItem(s, "", newDefaultTransfersDir, QString(), onSettingsUpgrade);
...
void Settings::loadPersonal(Profile* profile)
{
    ...
    ps.beginGroup("Friends");
    {
        ...
        auto autoAcceptDirUpgrader = UpgradableSetting<QString, QString>::Builder()
                                         .oldSettingsName("autoAcceptDir")
                                         .newSettingsName("autoAcceptDir")
                                         .upgradeHasHappenedName("autoAcceptUpgraded")
                                         .upgradeMessage(QString(tr(friendAutoAcceptUpgradeMessage)))
                                         .defaultConstructedIsNull()
                                         .build();
        ...
        // Batch the requests to upgrade all dirs at once
