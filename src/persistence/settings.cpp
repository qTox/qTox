...
bool Settings::verifyProxySettings(const QCommandLineParser& parser)
{
    ...
    if (parser.isSet("I")) {
        if (!(IPv6SettingString == ON
              || IPv6SettingString == OFF)) {
    ...
    if (parser.isSet("U")) {
        if (!(UDPSettingString == ON
              || UDPSettingString == OFF)) {
    ...
    if (parser.isSet("L")) {
        if (!(LANSettingString == ON
              || LANSettingString == OFF)) {
    ...
    if (parser.isSet("P")) {
        ...
        if (!(proxySettingStrings[0] == SOCKS5
              || proxySettingStrings[0] == HTTP)) {
...
bool Settings::applyCommandLineOptions(const QCommandLineParser& parser)
{
    ...
    if (parser.isSet("L")) {
        enableLanDiscovery = LANSetting == ON;
    ...
    if (parser.isSet("P")) {
        if (proxySettings[0] == NONE) {
            ...
            proxyPort = 0;
            return true;
        } else if (proxySettings[0] == SOCKS5) {
        ...
        proxyPort = static_cast<quint16>(portNumber);
