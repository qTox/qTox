...
bool createCurrentSchema(RawDatabase& db)
{
    ...
    queries += RawDatabase::Query(QStringLiteral(
        ...
        "CREATE TABLE faux_offline_pending (id INTEGER PRIMARY KEY, required_extensions INTEGER "
        "NOT NULL DEFAULT 0);"
        "CREATE TABLE broken_messages (id INTEGER PRIMARY KEY, reason INTEGER NOT NULL DEFAULT 0);"));
...
void dbSchemaUpgrade(std::shared_ptr<RawDatabase>& db)
{
    ...
    assert(databaseSchemaVersion < upgradeFns.size());
    for (int64_t i = databaseSchemaVersion; i < upgradeFns.size(); ++i) {
