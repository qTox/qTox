...
bool isNewDb(std::shared_ptr<RawDatabase>& db, bool& bSuccess)
{
    bool newDb;
...
bool dbSchema1to2(std::shared_ptr<RawDatabase>& db)
{
...
void dbSchemaUpgrade(std::shared_ptr<RawDatabase>& db)
{
    ...
    case 0: {
        ...
        bool bSuccess;
        const bool bIsNewDb = isNewDb(db, bSuccess);
        if (!bSuccess) {
...
QList<History::HistMessage> History::getBrokenMessagesForFriend(const ToxPk& friendPk)
{
    auto queryText =
        QString("SELECT history.id, faux_offline_pending.id, timestamp, chat.public_key, "
                "aliases.display_name, sender.public_key, message, broken_messages.id "
                ...
                "JOIN aliases on sender_alias = aliases.id "
                "JOIN peers sender on aliases.owner = sender.id "
                "JOIN broken_messages ON history.id = broken_messages.id "
                "WHERE chat.public_key='%1';")
            .arg(friendPk.toString());
