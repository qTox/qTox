...
/**
 * @var Identicon::IDENTICON_ROWS specifies how many rows of blocks the identicon should have
 */

/**
 * @var Identicon::ACTIVE_COLS width from the center to the outside, for 5 columns it's 3, 6 -> 3, 7
 * -> 4
 */
...
Identicon::Identicon(const QByteArray& data)
{
    ...
    for (int colorIndex = 0; colorIndex < COLORS; colorIndex++) {
        QByteArray hashPart = hash.right(IDENTICON_COLOR_BYTES);
        hash.truncate(hash.length() - IDENTICON_COLOR_BYTES);

        float hue = bytesToColor(hashPart);
        // change offset when COLORS != 2
        float lig = static_cast<float>(colorIndex) / COLORS + 0.3;
        float sat = 0.5;
        colors[colorIndex].setHslF(hue, sat, lig);
    }

    const uint8_t* hashBytes = reinterpret_cast<const uint8_t*>(hash.constData());
    ...
    for (int row = 0; row < IDENTICON_ROWS; row++) {
        for (int col = 0; col < ACTIVE_COLS; col++) {
            ...
            uint8_t colorIndex = hashBytes[hashIdx] % COLORS;
...
float Identicon::bytesToColor(QByteArray bytes)
{
    ...
    const uint8_t* bytesChr = reinterpret_cast<const uint8_t*>(bytes.constData());
