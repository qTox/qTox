...
void ProfileForm::onDeletePasswordFinished(){
    ...
    if (job->error())
        GUI::showWarning(tr("Deleting password in keychain failed", "Title of QtKeychain error message"), tr("Deleting the password in keychain failed: %1", "text of QtKeychain error message").arg(job->errorString()));
}
