...
namespace {
    ...
    {
        auto controlSize = option.rect.height() * 0.8;
        auto rectWidth = option.rect.width();
        auto buttonHorizontalArea = rectWidth / 2;
        // To center the button, we find the horizontal center and subtract half
        // our width from it
        auto buttonXPos = option.rect.x() + buttonHorizontalArea / 2 - controlSize / 2;
        auto buttonYPos = option.rect.y() + option.rect.height() * 0.1;
    ...
    QRect stopRect(const QStyleOptionViewItem& option)
    {
        ...
        // To center the button, we find the horizontal center and subtract half
        // our width from it
        auto buttonXPos = option.rect.x() + buttonHorizontalArea + buttonHorizontalArea / 2 - controlSize / 2;
        auto buttonYPos = option.rect.y() + option.rect.height() * 0.1;
        return QRect(buttonXPos, buttonYPos, controlSize, controlSize);
    }
    ...
    : QObject()
{
    ...
    main.addTab(new FileTransferList::View(recvdModel), QString());
    main.addTab(new FileTransferList::View(sentModel), QString());
