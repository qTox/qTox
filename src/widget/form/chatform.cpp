...
ChatForm::ChatForm(Friend chatFriend, QWidget* parent)
    ...
    , isTyping(false)
{
    ...
    connect(callButton, &QAbstractButton::clicked,
            this, &ChatForm::onCallButtonClicked);
    ...
    connect(Friend::notify(), &FriendNotify::nameChanged,
            this, &ChatForm::onFriendNameChanged);
    connect(Friend::notify(), &FriendNotify::statusChanged,
            this, &ChatForm::onFriendStatusChanged);
    connect(Friend::notify(), &FriendNotify::statusMessageChanged,
            this, &ChatForm::onStatusMessage);
    connect(Friend::notify(), &FriendNotify::loadHistory,
            this, &ChatForm::onLoadChatHistory);
    connect(Friend::notify(), &FriendNotify::avatarChanged,
            this, &ChatForm::onAvatarChange);
    connect(Friend::notify(), &FriendNotify::typingChanged,
...
void ChatForm::onAvInvite(Friend::ID friendId, bool video)
{
       ...
       (!video && Settings::getInstance().getAutoAcceptCall(f.getToxId()).testFlag(Settings::AutoAcceptCall::Audio)))
    {
        ...
        QMetaObject::invokeMethod(av, "answerCall", Qt::QueuedConnection, Q_ARG(uint32_t, friendId));
...
void ChatForm::onCallButtonClicked()
{
    CoreAV* av = Core::getInstance()->getAv();
...
void ChatForm::onFriendStatusChanged(const Friend& frnd, Status status)
{
    if(frnd.getFriendId() != f.getFriendId())
        return;
...
void ChatForm::onFriendTypingChanged(const Friend& frnd, bool isTyping)
{
    if (frnd.getFriendId() == f.getFriendId())
        setFriendTyping(isTyping);
}

void ChatForm::onFriendNameChanged(const Friend& frnd, const QString& name)
{
    if (frnd.getFriendId() == f.getFriendId())
        setName(name);
}
...
void ChatForm::onStatusMessage(const Friend& frnd, const QString& message)
{
    if (frnd.getFriendId() == f.getFriendId())
...
void ChatForm::onAvatarChange(const Friend& frnd, const QPixmap &pic)
{
    if (frnd.getFriendId() == f.getFriendId())
...
GenericNetCamView *ChatForm::createNetcam()
{
    ...
    VideoSource* source = coreAV->getVideoSourceFromCall(f.getFriendId());
...
void ChatForm::SendMessageStr(QString msg)
{
    ...
    for (CString& c_msg : splittedMsg)
    {
        ...
        {
            QString fPK = f.getToxId().publicKey;
            ...
            profile->getHistory()->addNewMessage(
                        fPK, qt_msg_hist, selfPK, timestamp,
                        status, name, [this, rec, ma](int64_t id)
            {
