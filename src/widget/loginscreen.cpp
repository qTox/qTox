    ...
    , quitShortcut{QKeySequence(Qt::CTRL + Qt::Key_Q), this}
{
    ...
    connect(ui->autoLoginCB, &QCheckBox::stateChanged, this, &LoginScreen::autoLoginToggled);
 ...
 */
void LoginScreen::reset(QString initialProfileName)
{
    ...
    QStringList allProfileNames = Profile::getAllProfileNames(); // static utility function
...
void LoginScreen::onProfileLoaded(bool success)
{
    if (success) {
        done(0);
    } else {
...
void LoginScreen::autoLoginToggled(int state)
{
    auto cstate = static_cast<Qt::CheckState>(state);
    emit setAutoLogin(cstate == Qt::CheckState::Checked);
}
...
{
    auto pi = new ProfileImporter(this);
