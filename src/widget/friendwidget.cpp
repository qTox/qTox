...
void FriendWidget::onContextMenuCalled(QContextMenuEvent* event)
{
    ...
    const bool notAlone = contentDialog && contentDialog->chatroomWidgetCount() > 1;
    if (!contentDialog || notAlone) {
        openChatWindow = menu.addAction(tr("Open chat in new window"));
    }

    const bool hasFriendWidget = contentDialog && contentDialog->hasFriendWidget(friendId, this);
