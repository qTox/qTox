...
ContentDialog::WidgetHash ContentDialog::friendList;
ContentDialog::WidgetHash ContentDialog::groupList;
...
FriendWidget* ContentDialog::addFriend(Friend::ID friendId)
{
    Friend f = Friend::get(friendId);
    FriendWidget* friendWidget = new FriendWidget(f, nullptr);
...
GroupWidget* ContentDialog::addGroup(int groupId)
{
    Group* g = Group::get(groupId);
    GroupWidget* groupWidget = new GroupWidget(g, nullptr);
    groupLayout.addSortedWidget(groupWidget);

    connect(groupWidget, &GroupWidget::chatroomWidgetClicked,
            this, &ContentDialog::onChatroomWidgetClicked);
...
void ContentDialog::cycleContacts(bool forward, bool loop)
{
    ...
    switch (activeChatroomWidget->type())
    {
...
void ContentDialog::updateTitleAndStatusIcon(const QString& username)
{
    if (displayWidget)
    {
        ...
        setWindowTitle(displayWidget->getTitle() + QStringLiteral(" - ") +
                       username);

        if (displayWidget->type() == GenericChatroomWidget::Type::Friend)
        {
            const FriendWidget* fw =
                    static_cast<FriendWidget*>(displayWidget.data());
            Friend f = fw->getFriend();
...
bool ContentDialog::event(QEvent* event)
{
    ...
    case QEvent::WindowActivate:
        if (activeChatroomWidget)
        {
            ...
            {
                const FriendWidget* fw =
                        static_cast<FriendWidget*>(activeChatroomWidget.data());
                emit friendDialogShown(fw->getFriend());
                break;
            }
            case GenericChatroomWidget::Type::Group:
                const GroupWidget* gw =
                        static_cast<GroupWidget*>(activeChatroomWidget.data());
                emit groupDialogShown(gw->getGroup());
                break;
            }
        }
...
void ContentDialog::dragEnterEvent(QDragEnterEvent *event)
{
    ...
    if (frnd)
    {
        ...
        auto iter = friendList.find(contact.getFriendId());
...
void ContentDialog::onChatroomWidgetClicked(GenericChatroomWidget *widget, bool group)
{
    if (group)
    {
        ...
        case GenericChatroomWidget::Type::Friend:
        {
            ...
            if (f)
                Widget::getInstance()->addFriendDialog(f, contentDialog);

            break;
        }
        case GenericChatroomWidget::Type::Group:
        {
            const GroupWidget* gw = static_cast<GroupWidget*>(widget);
            const Group* g = gw->getGroup();

            if (g)
                Widget::getInstance()->addGroupDialog(g, contentDialog);
    ...
    if (activeChatroomWidget)
        activeChatroomWidget->setAsActiveChatroom(false);
...
bool ContentDialog::hasWidget(uint32_t id, GenericChatroomWidget* chatroomWidget,
                              const WidgetHash& list)
{
...
bool ContentDialog::existsWidget(uint32_t id, bool focus, const WidgetHash& list)
{
    auto iter = list.find(id);
    if (iter == list.end())
        return false;
...
void ContentDialog::updateStatus(uint32_t id, const WidgetHash& list)
{
    auto iter = list.find(id);
...
bool ContentDialog::isWidgetActive(uint32_t id, const WidgetHash& list)
{
    auto iter = list.find(id);
...
ContentDialog* ContentDialog::getDialog(uint32_t id, const WidgetHash& list)
{
    auto iter = list.find(id);
