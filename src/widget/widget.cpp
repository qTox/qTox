...
void Widget::init()
{
    ...
    for (auto friendId : Friend::idList())
    {
        onFriendAdded(Friend::get(friendId));
    }
    ...
    connect(core, &Core::failedToStart,         this, &Widget::onFailedToStartCore, Qt::BlockingQueuedConnection);
    connect(core, &Core::badProxy,              this, &Widget::onBadProxyCore, Qt::BlockingQueuedConnection);
    connect(core, &Core::statusSet,             this, &Widget::onStatusSet);
    connect(core, &Core::usernameSet,           this, &Widget::setUsername);
    connect(core, &Core::statusMessageSet,      this, &Widget::setStatusMessage);
    connect(core, &Core::selfAvatarChanged,     this, &Widget::onSelfAvatarLoaded);
    connect(core, &Core::friendshipChanged,     this, &Widget::onFriendshipChanged);
    ...
    connect(&s, &Settings::contentArrangementChanged,
            this, &Widget::onContentArrangementChanged);

    connect(Friend::notify(), &FriendNotify::added,
            this, &Widget::onFriendAdded);
    ...
    onContentArrangementChanged(s.getContentArrangement());
}
...
/**
 * @brief Flips the widget arrangement in "embedded window" mode.
 * @param[in] arrangement   the new left/right arrangement
 */
void Widget::onContentArrangementChanged(Qt::Edge arrangement)
{
    if (Settings::getInstance().getSeparateWindow())
        return;
...
void Widget::onFriendAdded(Friend f)
{
    FriendWidget* widget = contactListWidget->requestFriendWidget(f, 0);
...
void Widget::onChatroomWidgetClicked(GenericChatroomWidget *widget, bool newWindow)
{
    ...
    widget->setEventFlag(false);

    switch (widget->type())
    {
    case GenericChatroomWidget::Type::Friend:
    {
        const FriendWidget* fw = static_cast<FriendWidget*>(widget);
        chatWidget = new ChatForm(fw->getFriend());
        break;
    }
    case GenericChatroomWidget::Type::Group:
    {
        GroupWidget* gw = static_cast<GroupWidget*>(widget);
        chatWidget = new GroupChatForm(gw->getGroup());
        break;
    }
    }

    activeChat = chatWidget;
    widget->setAsActiveChatroom(true);
    showContentWidget(chatWidget, chatWidget->windowTitle());
}

void Widget::onFriendMessageReceived(Friend::ID friendId,
...
void Widget::addFriendDialog(const Friend& f, ContentDialog *dialog)
{
    FriendWidget* widget = contactListWidget->getFriendWidget(f.getFriendId());
    FriendWidget* friendWidget = dialog->addFriend(f.getFriendId());

    friendWidget->setStatusMsg(widget->getStatusMsg());

    connect(friendWidget, SIGNAL(copyFriendIdToClipboard(uint32_t)),
            this, SLOT(copyFriendIdToClipboard(uint32_t)));
...
void Widget::addGroupDialog(const Group* g, ContentDialog *dialog)
{
    ...
    GroupWidget* widget = contactListWidget->getGroupWidget(g->getGroupId());
    if (!contentDialog && !separate)
        onAddClicked();

    GroupWidget* groupWidget = dialog->addGroup(g->getGroupId());
...
bool Widget::newFriendMessageAlert(Friend::ID friendId, bool sound)
{
    ...
    else
    {
        ...
        else
        {
            ...
            FriendWidget* fw = contactListWidget->getFriendWidget(friendId);
            hasActive = fw->isActive();
        }
    }
    ...
    {
        FriendWidget *fw = contactListWidget->getFriendWidget(friendId);
        fw->setEventFlag(true);
        fw->updateStatusLight();
        friendList->trackWidget(fw);

        if (contentDialog == nullptr)
        {
            if (hasActive)
            {
                setWindowTitle(fw->getTitle());
            }
        }
        else
        {
...
bool Widget::newGroupMessageAlert(int groupId, bool notify)
{
    ...
    {
        currentWindow = window();
        GroupWidget* gw = contactListWidget->getGroupWidget(groupId);
        hasActive = gw->isActive();
    }

    if (newMessageAlert(currentWindow, hasActive, true, notify))
    {
        GroupWidget* gw = contactListWidget->getGroupWidget(groupId);
        gw->setEventFlag(true);
        gw->updateStatusLight();

        if (contentDialog == nullptr)
        {
            if (hasActive)
            {
                setWindowTitle(gw->getTitle());
            }
        }
        else
        {
...
void Widget::updateFriendActivity(Friend f)
{
    ...
    if (date != QDate::currentDate())
    {
        ...
        QDate oldDate = Settings::getInstance().getFriendActivity(f.getToxId());
        Settings::getInstance().setFriendActivity(f.getToxId(), QDate::currentDate());
        contactListWidget->moveWidget(
                    contactListWidget->getFriendWidget(f.getFriendId()),
                    f.getStatus());
...
void Widget::onFriendDialogShown(Friend f)
{
    ...
    onDialogShown(contactListWidget->getFriendWidget(friendId));
}

void Widget::onGroupDialogShown(Group *g)
{
    onDialogShown(contactListWidget->getGroupWidget(g->getGroupId()));
}
...
void Widget::onGroupMessageReceived(int groupId, int peernumber,
                                    const QString& message, bool isAction)
{
    Q_UNUSED(isAction);

    const Core* core = Core::getInstance();
    ToxId author = core->getGroupPeerToxId(groupId, peernumber);
    bool isSelf = author == core->getSelfId();
    bool targeted = !isSelf && (message.contains(nameMention) ||
                                message.contains(sanitizedNameMention));

    newGroupMessageAlert(groupId, targeted ||
                         Settings::getInstance().getGroupAlwaysNotify());
}
...
{
    GroupList::removeGroup(g->getGroupId());

    ContentDialog* contentDialog = ContentDialog::getGroupDialog(g->getGroupId());
    if (contentDialog)
        contentDialog->removeGroup(g->getGroupId());

    Core::getInstance()->removeGroup(g->getGroupId(), fake);
...
Group* Widget::createGroup(int groupId)
{
    ...
    bool avAvailable = av && av->isGroupAvEnabled(groupId);
    Group* newgroup = GroupList::addGroup(groupId, groupName, avAvailable);
    GroupWidget* gw = contactListWidget->requestGroupWidget(groupId);
    gw->updateStatusLight();

    connect(gw, &GroupWidget::chatroomWidgetClicked,
            this, &Widget::onChatroomWidgetClicked);

    FilterCriteria filter = getFilterCriteria();
    gw->searchName(searchContactText->text(), filterGroups(filter));
...
void Widget::onEmptyGroupCreated(int groupId)
{
    ...
    if (Widget::getInstance()->groupsVisible())
        contactListWidget->getGroupWidget(groupId)->editName();
}
...
void Widget::processOfflineMsgs()
{
    if (OfflineMsgEngine::globalMutex.tryLock())
    {
        ...
        {
            Friend::get(friendId).deliverOfflineMsgs();
        }
...
void Widget::clearAllReceipts()
{
    for (Friend::ID friendId: Friend::idList())
    {
        Friend::get(friendId).clearOfflineReceipts();
    }
}
 ...
 * main window, depending on the position within the virtual desktop.
 */
