...
AppManager::AppManager(int argc_, char** argv_)
    : argc{(qInstallMessageHandler(logMessageHandler), argc_)}
    , argv{argv_}
...
int AppManager::run()
{
    ...
    if (ipc->isAttached()) {
        QObject::connect(settings.get(), &Settings::currentProfileIdChanged, ipc.get(), &IPC::setProfileId);
    } else {
    ...
    QObject::connect(qapp.get(), &QApplication::aboutToQuit, this, &AppManager::cleanup);
