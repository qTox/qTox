...
// Check whether we have an update waiting to be installed
    ...
    if (!(cacheOk && dataOk && settingsOk)) {
        ...
        GUI::showError("Disk error", "Error while reading disk. Proceed with caution!");
    }
    if (diskFull) {
        qCritical() << "Disk full, exiting!";
        GUI::showError("Disk error", "Disk full. Exiting.");
    }
    ...
    if (!diskFull && (CheckDisk::diskUsage(cacheDir, cacheOk) > DISK_WARN_THRESHOLD
        ...
        GUI::showError("Disk warning", "High disk usage. Consider freeing space.");
    }
