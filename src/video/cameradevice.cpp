...
CameraDevice* CameraDevice::open(QString devName, VideoMode mode)
{
    ...
    std::string videoSize = QString("%1x%2").arg(screen.width()).arg(screen.height()).toStdString();
    std::string framerate = QString().setNum(FPS).toStdString();
    ...
    if (!iformat)
        ...
        std::string offsetX = QString().setNum(mode.x).toStdString();
        std::string offsetY = QString().setNum(mode.y).toStdString();
        av_dict_set(&options, "framerate", framerate.c_str(), 0);
