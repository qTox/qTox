...
{
    ToxPk friendPk(cFriendPk, TOX_PUBLIC_KEY_SIZE);
...
void Core::loadFriends()
{
    ...
    if (friendCount > 0)
    {
        ...
        for (int32_t i = 0; i < static_cast<int32_t>(friendCount); ++i)
        {
            ...
            {
                emit friendAdded(ids[i], ToxPk(friendPk, TOX_PUBLIC_KEY_SIZE));
...
ToxPk Core::getGroupPeerKey(int groupId, int peerId) const
{
    ...
    return ToxPk(friendPk, TOX_PUBLIC_KEY_SIZE);
}
...
ToxPk Core::getFriendPublicKey(uint32_t friendNumber) const
{
    ...
    return ToxPk(rawid, TOX_PUBLIC_KEY_SIZE);
}
