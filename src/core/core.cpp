...
void Core::bootstrapDht()
{
    ...
    while (i < 2) // i think the more we bootstrap, the more we jitter because the more we overwrite nodes
    {
        ...
        const char* address = dhtServer.address.toLatin1().constData();
        const uint8_t* pk = reinterpret_cast<const uint8_t*>(ToxId{dhtServer.userId}.getPublicKey().constData());

        if (!tox_bootstrap(tox, address, dhtServer.port, pk, nullptr))
        {
            qDebug() << "Error bootstrapping from "+dhtServer.name;
        }

        if (!tox_add_tcp_relay(tox, address, dhtServer.port, pk, nullptr))
        {
