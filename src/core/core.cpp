                                 ...
                                 TOX_CONFERENCE_STATE_CHANGE change, void* core)
{
    ...
    qDebug() << QString("Group namelist change %1:%2 %3")
                    .arg(QString::number(groupId), QString::number(peerId), QString::number(change));
...
QStringList Core::getGroupPeerNames(int groupId) const
{
    ...
    for (uint32_t i = 0; i < nPeers; ++i) {
        ...
        int length = tox_conference_peer_get_name_size(tox, groupId, i, &error);
