...
{
public:
    ToxOptionsWrapper(Tox_Options *options, QByteArray *proxyAddrData) {
        this->options = options;
        this->proxyAddrData = proxyAddrData;
    }
...
ToxOptionsWrapper initToxOptions(const QByteArray& savedata, const ICoreSettings* s)
{
    ...
    ToxOptionsWrapper toxOptions = ToxOptionsWrapper(tox_options_new(nullptr), new QByteArray(proxyAddrData));
    ...
    if (proxyType != ICoreSettings::ProxyType::ptNone) {
        if (proxyAddr.length() > MAX_PROXY_ADDRESS_LENGTH) {
            ...
            tox_options_set_proxy_host(toxOptions, proxyAddrData.data());
