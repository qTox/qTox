...
ToxEncrypt::ToxEncrypt(const QString& password)
{
    ...
    {
        qWarning() << ToxEncrypt::getKeyDerivationError(error);
        tox_pass_key_free(passKey);
        passKey = nullptr;
    }
}
...
QByteArray ToxEncrypt::encryptPass(const QString& password, const QByteArray& plaintext)
{
    ...
    {
        qWarning() << ToxEncrypt::getEncryptionError(error);
        return QByteArray{};
    }
 ...
 */
QString ToxEncrypt::getKeyDerivationError(TOX_ERR_KEY_DERIVATION error)
{
    switch(error)
    {
 ...
 * @return The verbose error message.
 */
QString ToxEncrypt::getEncryptionError(TOX_ERR_ENCRYPTION error)
{
    switch(error)
    {
 ...
 * @return The verbose error message.
 */
QString ToxEncrypt::getDecryptionError(TOX_ERR_DECRYPTION error)
{
    switch(error)
    {
 ...
 * @return The verbose error message.
 */
QString ToxEncrypt::getSaltError(TOX_ERR_GET_SALT error)
{
    switch(error)
    {
