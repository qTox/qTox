...
CoreAV::CoreAVPtr CoreAV::makeCoreAV(Tox* core)
{
    ...
    return CoreAVPtr(new CoreAV(*toxav));
}
...
bool CoreAV::startCall(uint32_t friendNum, bool video)
{
    ...
    ToxFriendCallPtr call{new ToxFriendCall{friendNum, video, *this}};
                           ...
                           uint32_t rate) const
{
    ...
    ToxFriendCallPtr const& call = it->second;
...
void CoreAV::sendCallVideo(uint32_t callId, std::shared_ptr<VideoFrame> vframe)
{
    ...
    ToxFriendCallPtr& call = it->second;
...
#else
        toxav_bit_rate_set(toxav.get(), callId, -1, VIDEO_DEFAULT_BITRATE, nullptr);
#endif
                               ...
                               unsigned samples, uint8_t channels, uint32_t sample_rate, void* core)
{
    ...
    ToxGroupCallPtr& call = it->second;
...
void CoreAV::joinGroupCall(int groupId)
{
    ...
    auto ret = groupCalls.emplace(groupId, std::move(call));
    if (ret.second == false) {
...
void CoreAV::sendNoVideo()
{
    ...
    for (auto& kv : calls) {
        ToxFriendCallPtr& call = kv.second;
#if TOX_VERSION_IS_API_COMPATIBLE(0, 2, 0)
        toxav_video_set_bit_rate(toxav.get(), kv.first, 0, nullptr);
#else
        toxav_bit_rate_set(toxav.get(), kv.first, -1, 0, nullptr);
#endif
...
void CoreAV::callCallback(ToxAV* toxav, uint32_t friendNum, bool audio, bool video, void* vSelf)
{
    ...
    ToxFriendCallPtr call{new ToxFriendCall{friendNum, video, *self}};
