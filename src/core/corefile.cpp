...
void CoreFile::sendAvatarFile(Core* core, uint32_t friendId, const QByteArray& data)
{
    ...
    if (data.isEmpty()) {
        TOX(file_send)(core->tox, friendId, TOX_FILE_KIND_AVATAR, 0, nullptr, nullptr, 0, nullptr);
        return;
    }

    static_assert(TOX_HASH_LENGTH <= TOX_FILE_ID_LENGTH, "TOX_HASH_LENGTH > TOX_FILE_ID_LENGTH!");
    uint8_t avatarHash[TOX_HASH_LENGTH];
    TOX(hash)(avatarHash, (uint8_t*)data.data(), data.size());
    uint64_t filesize = data.size();

    TOX_ERR_FILE_SEND error;
    uint32_t fileNum = TOX(file_send)(core->tox, friendId, TOX_FILE_KIND_AVATAR, filesize,
    ...
    TOX(file_get_file_id)(core->tox, friendId, fileNum, (uint8_t*)file.resumeFileId.data(), nullptr);
                        ...
                        long long filesize)
{
    ...
    uint32_t fileNum = TOX(file_send)(core->tox, friendId, TOX_FILE_KIND_DATA, filesize, nullptr,
...
void CoreFile::pauseResumeFileSend(Core* core, uint32_t friendId, uint32_t fileId)
{
    ...
    if (file->status == ToxFile::TRANSMITTING) {
        ...
        TOX(file_control)(core->tox, file->friendId, file->fileNum, TOX_FILE_CONTROL_PAUSE, nullptr);
    } else if (file->status == ToxFile::PAUSED) {
        file->status = ToxFile::TRANSMITTING;
        emit core->fileTransferAccepted(*file);
        TOX(file_control)(core->tox, file->friendId, file->fileNum, TOX_FILE_CONTROL_RESUME, nullptr);
    } else {
...
void CoreFile::cancelFileSend(Core* core, uint32_t friendId, uint32_t fileId)
{
    ...
    TOX(file_control)(core->tox, file->friendId, file->fileNum, TOX_FILE_CONTROL_CANCEL, nullptr);
                                     ...
                                     void* vCore)
{
    ...
    if (kind == TOX_FILE_KIND_AVATAR) {
        ...
        if (!filesize) {
            ...
            TOX(file_get_file_id)(core->tox, friendId, fileId, avatarHash, nullptr);
            if (core->profile.getAvatarHash(friendAddr)
                ...
                qDebug() << QString(
                                ...
                                .arg(friendId)
                                .arg(fileId);
                TOX(file_control)(core->tox, friendId, fileId, TOX_FILE_CONTROL_CANCEL, nullptr);
                return;
            } else {
                ...
                qDebug() << QString("Received avatar request %1:%2, accept, since we don't have it "
                                ...
                                .arg(friendId)
                                .arg(fileId);
                TOX(file_control)(core->tox, friendId, fileId, TOX_FILE_CONTROL_RESUME, nullptr);
            }
        }
    } else {
    ...
    file.fileKind = kind;
    file.resumeFileId.resize(TOX_FILE_ID_LENGTH);
    TOX(file_get_file_id)(core->tox, friendId, fileId, (uint8_t*)file.resumeFileId.data(), nullptr);
                                  ...
                                  size_t length, void* core)
{
    ...
    if (file->fileKind == TOX_FILE_KIND_AVATAR) {
        ...
        if (nread <= 0) {
            ...
            emit static_cast<Core*>(core)->fileTransferCancelled(*file);
            TOX(file_send_chunk)(tox, friendId, fileId, pos, nullptr, 0, nullptr);
            removeFile(friendId, fileId);
            return;
        }
    ...
    if (!TOX(file_send_chunk)(tox, friendId, fileId, pos, data.get(), nread, nullptr)) {
                                       ...
                                       const uint8_t* data, size_t length, void* vCore)
{
    ...
    if (!file) {
        ...
        TOX(file_control)(tox, friendId, fileId, TOX_FILE_CONTROL_CANCEL, nullptr);
        return;
    }
