                                     ...
                                     void* vCore)
{
    ...
    if (dirtyfilename.contains(regex))
    {
        filename.replace(filename, (core->getCleanFileName(filename)));
        qDebug() << QString("Cleaned filename from %1 to %2").arg(dirtyfilename).arg(QString::fromStdString(filename.toStdString()));
    } else {
