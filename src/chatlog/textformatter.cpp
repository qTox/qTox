...
void TextFormatter::applyHtmlFontStyling(bool showFormattingSymbols)
{
    processUrl(message, [] (QString& str) {
        for (char c : MARKDOWN_SYMBOLS) {
            str.replace(c, HTML_CHARACTER_CODE.arg(QString::number(static_cast<int>(c))));
        }
        return str;
    });
    ...
    for (char c : MARKDOWN_SYMBOLS) {
        message.replace(HTML_CHARACTER_CODE.arg(QString::number(static_cast<int>(c))), QString(c));
    }
}
