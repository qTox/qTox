...
static const QVector<QPair<QString, QString>> TextFormatter::textPatternStyle {
...
QString TextFormatter::applyHtmlFontStyling(bool dontShowFormattingSymbols){
    ...
    for (QPair pair : textPatternStyle) {
        ...
        while (matchesIterator.hasNext()) {
            ...
            int operationStart = matchStart + choppingSignsCount;
            int operationLength = matchLength - choppingSignsCount * 2;

            QString styledText = pair->second.arg(out.mid(operationStart, operationLength));

            operationStart = matchStart + choppingSignsCount * choppingSignsCountMultiplier;
            operationLength = matchLnegth - choppingSignsCount * choppingSignsCountMultiplier * 2;

            out.replace(operationStart, operationLength, styledText);
