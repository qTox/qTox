...
std::vector<IChatLog::DateChatLogIdxPair> ChatHistory::getDateIdxs(const QDate& startDate, size_t maxDates) const
{
    if (canUseHistory()) {
...
void ChatHistory::onFileUpdated(const ToxPk& sender, const ToxFile& file)
{
    if (canUseHistory()) {
        ...
        case ToxFile::INITIALIZING: {
            ...
            history->addNewFileMessage(f.getPublicKey().toString(), file.resumeFileId,
                                       file.fileName, file.filePath, file.filesize, sender.toString(),
...
void ChatHistory::loadHistoryIntoSessionChatLog(ChatLogIdx start) const
{
    ...
    for (const auto& message : messages) {
        ...
        case HistMessageContentType::message: {
            ...
            // It's okay to skip the message processor here because we can
            // assume any messages that went into history were already
            // processed
