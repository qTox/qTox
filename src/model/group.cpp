...
{
    auto departedPks = oldPks;
    for (auto tIt = newPks.begin(); tIt != newPks.end(); ++tIt) {
        const auto idx = departedPks.indexOf(tIt.key());
        if (idx != -1) {
            departedPks.removeAt(idx);
        }
    }
    Core* core = Core::getInstance();
    for (const auto& pk : departedPks) {
