...
bool OpenAL::initInput(const QString& deviceName, uint32_t channels)
{
    ...
    const ALCsizei bufSize = AUDIO_FRAME_SAMPLE_COUNT_TOTAL * bytesPerSample * safetyFactor;
...
{
    static const quint32 samples = AUDIO_FRAME_SAMPLE_COUNT_TOTAL;
    static const float twoOverRootTwo = 2/std::sqrt(2);
    ...
    const float normalizedVolume = rms * twoOverRootTwo;
