...
bool Platform::setAutorun(bool on)
{
    ...
    if (on) {
        ...
        result = RegSetValueEx(key, keyName.c_str(), 0, REG_SZ, const_cast<BYTE*>(reinterpret_cast<const BYTE*>(path.c_str())),
...
bool Platform::getAutorun()
{
    ...
    if (RegQueryValueEx(key, keyName.c_str(), 0, &type, const_cast<BYTE*>(reinterpret_cast<const BYTE*>(path)), &length) == ERROR_SUCCESS
        && type == REG_SZ)
        result = true;
