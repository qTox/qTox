...
- Merge the Flatpak manifest of our [Flathub repository] into
  [`./flatpak/io.github.qtox.qTox.json`]. Change the URL field of qTox to
  `"path": "/build/qtox/"`, and update the tag name and commit hash.
...
- Download the binaries that are part of the draft release, sign them in
  in detached and ascii armored mode, i.e. `-a -b`, and upload the signatures to
  the draft release.
...
- Modify the flatpak manifest created before tag to replace the qTox path back
  to a URL to the repo. Use this version to open a PR to update the Flatpak
  manifest of our [Flathub repository]. Do not commit this version in the qTox
  repo.
...
- After the build passed for qTox on all architectures on
  ...
  [Flathub repository].
- Update [`./flatpak/io.github.qtox.qTox.json`] to replace the tag with
  `"branch": "<current branch>"`, and remove the commit version.
